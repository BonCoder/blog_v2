		...
		<el-card class="box-card">
			...
			<el-row type="flex" class="el-col-24 recommend-item" justify="space-between" v-for="(item,index) in recommends" :key="index">
					<el-col :span="4" class="item">
						<div class="">
							<el-image :src="item.thumb" alt="Mozilan" class="profile-pic"></el-image>
						</div>
					</el-col>
					<el-col :span="18" style="padding-left:10px">
						<div class="">
                            <router-link :to="{name:'查看文章',params: {art_id:item.id}}">
							    <h3 title="Mozilan" class="name"> {{item.title}}</h3>
                            </router-link>
...
<script>
	export default {
		...
		computed:{
			recommends(){
				return this.$store.getters.getRecommendArticles;
			}
		}
	}
</script>